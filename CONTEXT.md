...
8. Сообщение коммита — описательное, по стилю репозитория.
...
- handlers/
  ...
  - briefing.py — generate_briefing (asyncio.gather по 8 источникам, 3 раздела синтеза)
- services/
...
- **/briefing**: 8 источников (Hacker News, HN-AI/Algolia, Lobsters, HF Papers,
  arXiv, Dev.to, Google News RSS, Tavily — опционален), дедуп по заголовкам между
  источниками, кэш сырых источников (`BRIEFING_CACHE_TTL`), отдельная модель
  (`settings.briefing_model`). Один вызов OpenRouter синтезирует из ВСЕХ источников
  3 раздела: 🔥 Главная новость дня / 💬 Что сейчас обсуждают / 👀 На что обратить
  внимание. Блок «Источники» с **кликабельными заголовками** `[title](url)`,
  собранными в коде (не LLM) → `parse_mode="Markdown"`, `disable_web_page_preview=True`.
  При сбое LLM (`ExternalAPIError`) вместо пустой ошибки возвращается шапка + блок
  собранных ссылок (`_briefing_fallback`). Пустой результат (нет источников) →
  `BRIEFING_EMPTY_MSG` без вызова LLM.
...
```
Базовое состояние: **65 passed**, ruff/mypy чисты.
