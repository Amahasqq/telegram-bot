...
- Бот **не выставляет `parse_mode`** ни в одном ответе, КРОМЕ `/briefing`
  (блок «Источники» с кликабельными заголовками `[title](url)`, собранными в
  коде из структурированных данных — разметка всегда валидна). В остальных
  ответах битый Markdown тишину НЕ вызывает.
...
- `handlers/`
  ...
  - `briefing.py` — `generate_briefing` — `asyncio.gather` по 8 источникам (синтез в 3 раздела)
- `services/`
  - `memory.py` — `MemoryManager` (HF Dataset), sync, локи
  - `llm.py` — `call_openrouter` (ретраи), `extract_facts`
  - `search.py` — Tavily
  - `trends.py` — HN / HN-AI (Algolia) / HF Papers / Lobsters / arXiv / Dev.to / Google News
...
| HF Daily Papers / Lobsters / arXiv / Google News / Dev.to | нет | ✅ источники `/briefing` |
